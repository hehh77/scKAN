...
class FastKAN(nn.Module):
    def __init__(
        ...
        self.n_classes = layers_hidden[-1]
...
class AttentionWithFastKANTransform_new(nn.Module):
    ...
    def __init__(
        ...
        super(AttentionWithFastKANTransform_new, self).__init__()
    ...
    def forward(
        ...
        att = torch.einsum('...qohc,...okhc->...qkh', wq, wk).softmax(-2)
        ...
        o = torch.einsum('...qkh,...okhc->...qhc', att, wv)       # *qhc
        del att, wv


        o = o.reshape(o.shape[0],o.shape[1], o.shape[-1] * o.shape[-2])           # *q(hc)
...
class kan_encoder_layer(nn.Module):
    def __init__(
        self,
        d_model: int,
        nhead: int,
        ...
        super(kan_encoder_layer, self).__init__()
        self.use_layernorm = use_layernorm
        self.use_gating = use_gating
        self.fast_transformer_backend = fast_transformer_backend
        self.nhead = nhead
        self.d_model = d_model
        self.attention = AttentionWithFastKANTransform_new(
            d_model, d_model, d_model, d_model // nhead, nhead, gating=use_gating
        )
        self.feedforward = feed_forward(d_model, dim_feedforward, dropout, activation)
...
class feed_forward(nn.Module):
    def __init__(
        ...
        self.liner1 = nn.Linear(d_model, dim_feedforward)
        self.activation = activation
        self.liner2 = nn.Linear(dim_feedforward, d_model)
    ...
    def forward(self, src: torch.Tensor) -> torch.Tensor:
        src2 = self.liner1(src)
        src2 = self.activation(src2)
        src2 = self.dropout1(src2)
        src2 = self.liner2(src2)
        src = src + self.dropout2(src2)
        src = self.layernorm2(src)
        return src
...
class Kan_TransformerModel(nn.Module):
    def __init__(
        self,
        ntoken: int,
        d_model: int,
        ...
        self.transformer_encoder = kan_encoder_layer(d_model, nhead, d_hid, dropout)
        ...
        # )
        self.cls_decoder = ClsDecoder_with_hidden(d_model, n_cls, nlayers=nlayers_cls)
        # if do_mvc:
    ...
    def forward(
        ...
        if CLS:
            output['hidden'], output["cls_output"] = self.cls_decoder(cell_emb)  # (batch, n_cls)
        # if CCE:
        #     cell1 = cell_emb
...
class ClsDecoder_with_hidden(nn.Module):
    """
    Decoder for classification task.
    """
