# this script is partially based on the scGPT repository (https://github.com/bowang-lab/scGPT). Thanks to the original authors.
import copy
import json
import os
...
run = wandb.init(
    ...
    project="teacher_model_finetuning_muto",
    reinit=True,
...
save_dir = Path(f"./save/muto_llm_finetuning_{config.data_name}-{time.strftime('%b%d-%H-%M')}/")
...
data_dir = Path(f"../data/{config.data_name}")
...
(
    ...
    all_counts, celltypes_labels, batch_ids, test_size=1/8, shuffle=True
)
    ...
    for batch, batch_data in enumerate(loader):
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            logger.info(
                ...
                f"lr {lr:5.6f} | ms/batch {ms_per_batch:5.2f} | "
...
if 'neighbors' not in adata_test_raw.uns:
    sc.pp.neighbors(adata_test_raw, n_neighbors=10, n_pcs=40)
...
# save the pred_h5ad
adata_test_raw.write(save_dir / "pred.h5ad")
